 ...
 *
 */
public class GuestVMProcessFilter {
    ...
    static class State {
        State(GuestVMProcessFilter filter, int fd) {
            _filter = filter;
            _fd = fd;
        }
        GuestVMProcessFilter _filter;
        int _fd;
        byte[] _data;
    }
     ...
     * @return
     */
    public static GuestVMProcessFilter getFilter(int key) {
     ...
     */
    protected GuestVMProcessFilter() {

    }

    protected GuestVMProcessFilter(String name) {
        _name = name;
    ...
    final int nextId() {
        ...
        _nextId += 3;
        final int[] fds = GuestVMProcessFilter.getFds(result);
        for (int i = 0; i < fds.length; i++) {
            final int fd = GuestVMProcessFilter.keyToFd(fds[i]);
    ...
    public static int invokeReadBytes(int key, byte[] bytes, int offset, int length, long fileOffset) {
        final int fd = GuestVMProcessFilter.keyToFd(key);
            ...
            if (prop != null) {
                ...
                for (String className : classNames) {
                    try {
                        ...
                        GuestVMError.unexpected("failed to load process filter class: " + prop);
                    }
                }
            }
            _init = true;
