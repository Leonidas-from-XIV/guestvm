...
unsigned char *guestvmXen_getMacAddress(void) {
...
unsigned char *guestvmXen_getNicName(void) {
  while (mac_address_set == 0) {
    sleep(1000);
  }
  return nic_name;
...
int guestvmXen_netStart(GUKNetDeviceCopyPacketMethod m) {
  while (mac_address_set == 0) {
    sleep(1000);
  }
...
void *net_dlsym(const char *symbol) {
    if (strcmp(symbol, "guestvmXen_getMacAddress") == 0) return guestvmXen_getMacAddress;
    else if (strcmp(symbol, "guestvmXen_getNicName") == 0) return guestvmXen_getNicName;
    else if (strcmp(symbol, "guestvmXen_netStart") == 0) return guestvmXen_netStart;
    else return 0;
}
